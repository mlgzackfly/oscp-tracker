...
let ui = { editingPlan: false, noteId: null, noteQ: "", view: "overview", track: "OSCP", q: "", platform: "", os: "", status: "", level: "", sort: "list", requiredOnly: false, open: null, scope: "required", drawOs: "", drawLevel: "", skipDone: true, weekOffset: 0, current: null };
    ...
    <div class="detail-grid">
      ...
      <div class="field wide">
        ...
        <div style="display:flex;gap:10px;align-items:center">
          <span class="result-line" style="flex:1">${e.notes ? esc(e.notes.split("\n").find((l) => l.trim() && !l.startsWith("#"))?.trim().slice(0, 60) || (e.notes.length + " 字")) : "還沒寫筆記"}</span>
...
function noteMachines() {
  ...
  return MACHINES.filter((m) => {
    const e = peek(m.id);
    if (e.status === "todo" && !e.notes && !(e.phases || []).length) return false;
    if (!q) return true;
    return m.name.toLowerCase().includes(q) || (e.notes || "").toLowerCase().includes(q);
  }).sort((a, b) => {
    const ea = peek(a.id);
    const eb = peek(b.id);
    if (!!eb.notes !== !!ea.notes) return eb.notes ? 1 : -1;
...
function renderNotes() {
  ...
  listHost.innerHTML = list.length
    ? list
        ...
        .join("")
    : `<p class="empty" style="padding:20px;text-align:center">還沒有動過的靶機。標一台進行中，或用右上角的下拉直接挑一台開始寫。</p>`;
  ...
  editHost.innerHTML = `
    ...
    <textarea data-act="notes" data-id="${esc(m.id)}" placeholder="這台的攻擊面、卡在哪、下次怎麼更快。">${esc(e.notes || "")}</textarea>

    <div class="toolrow">
      ${!e.notes ? `<button class="btn sm" data-act="template" data-id="${esc(m.id)}">插入模板</button>` : ""}
      ...
      <span class="right">
        <span class="result-line">${e.notes ? e.notes.length + " 字" : ""}</span>
        <button class="btn sm" data-act="open-in-list" data-id="${esc(m.id)}">在清單中開啟</button>
      </span>
    </div>`;
}
...
function render() {
  ...
  const noteCount = Object.values(state.entries).filter((e) => e.notes).length;
  $("#nav-count-notes").textContent = noteCount || "";
}
